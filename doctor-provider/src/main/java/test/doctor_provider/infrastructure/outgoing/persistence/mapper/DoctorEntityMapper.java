...
public interface DoctorEntityMapper {
	...
	/*
	 * die Oberen Annotation brauchen wir hierh nicht , weil: MapStruct denkt sich:
	 * "Aha, ich soll eine Liste von DoctorEntity in eine Liste von Doctor
	 * umwandeln. Ich habe schon eine Methode, die ein einzelnes DoctorEntity →
	 * Doctor konvertiert. Ich rufe einfach diese Methode für jedes Element in der
	 * Liste auf!"
	 */
	...
	default Set<SpecialityEntity> specialityIdsToEntities(Set<UUID> specialityIds) {
		...
		return specialityIds.stream().map(id -> {
	...
	default Set<UUID> specialityEntitiesToIds(Set<SpecialityEntity> specialities) {
		...
		return specialities.stream().map(SpecialityEntity::getId).collect(Collectors.toSet());
	}
}
