 ...
 *
 * Mapping-Richtungen: - City → CityDto (toDto): Server → Client - List<City> →
 * List<CityDto>: Für paginierte API-Responses
 *
 * WICHTIG: - id wird NICHT ignoriert, weil der Client die ID sehen muss!
 */
...
public interface CityWebMapper {
	 ...
	 *
	 * Datenfluss: Server → Client (Response für GET /api/v1/internal/cities + /api/v1/external/cities)
	 *
	 * WICHTIG: - Diese Methode ist für die paginierte Liste in der API-Response -
